...
def _detect_tip_optical_flow(prev_gray, curr_gray, lm, frame_shape):
    ...
    try:
        ...
        s_wrist_x = wrist_x * scale
        s_wrist_y = wrist_y * scale

        # Dense optical flow（軽量パラメータ）
        ...
        if body_center is not None:
            ...
            s_body_cx = body_cx * scale
            s_body_cy = body_cy * scale
            out_dx = float(s_wrist_x - s_body_cx)
            out_dy = float(s_wrist_y - s_body_cy)
            out_len = np.sqrt(out_dx**2 + out_dy**2)
            if out_len > 5:
                out_dx /= out_len
                out_dy /= out_len
                rel_x = xs.astype(np.float32) - s_wrist_x
                rel_y = ys.astype(np.float32) - s_wrist_y
        ...
        mags = mag[ys, xs]
        dists = np.sqrt((xs.astype(np.float32) - s_wrist_x)**2 +
                        (ys.astype(np.float32) - s_wrist_y)**2)
        ...
        # 元の解像度に戻す
        tip_x = int(np.mean(xs[top]) / scale)
        tip_y = int(np.mean(ys[top]) / scale)
